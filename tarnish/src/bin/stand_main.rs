...
fn main() -> std::io::Result<()> {
    ...
    let spine1 = geom::Vec2 {
        x: wing_length * f64::cos(side_angle),
    ...
    let spine3 = geom::Vec2 {
        x: spine2.x + wing_length * f64::cos(side_angle),
        y: 0.0,
    };
    ...
    let baffle_lower_stand_hole_center_x =
        (air::BAFFLE_LL_STAND_HOLE_CENTER_X - air::BAFFLE_UL_STAND_HOLE_CENTER_X) / 2.0;
    let baffle_ll_stand_hole_x_offset_to_center_line =
        air::BAFFLE_LL_STAND_HOLE_CENTER_X - baffle_lower_stand_hole_center_x;
    let baffle_ll_stand_hole_y_offset_to_bend_line =
    ...
    let baffle_ll_hole_center = utils::transform_point(
        &left_bar_origin,
        &left_bar_x_axis,
        &left_bar_y_axis,
        &geom::Vec2 {
            x: baffle_ll_stand_hole_x_offset_to_center_line,
            y: baffle_ll_stand_hole_y_offset_to_bend_line,
        },
    );
    println!(
        "{}",
    ...
    let b0 = geom::Vec2 {
        x: s1_a.x + (s2_a.x - s1_a.x) / 2.0 - stand_back_width / 2.0,
        y: stand_depth,
    };
    let b1 = geom::Vec2 {
        x: s2_a.x - ((s2_a.x - s1_a.x) / 2.0 - stand_back_width / 2.0),
        y: stand_depth,
    };
